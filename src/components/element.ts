...
*/
import { App } from 'vue'
import { ElTabs, ElTabPane, ElButton, ElForm, ElFormItem, ElInput, ElRow, ElCol, ElCheckbox, ElRadioGroup, ElRadio, ElCheckboxGroup, ElInputNumber, ElSelect, ElOption, ElColorPicker } from 'element-plus'
...
export default function (app: App): App {
  return app.use(ElTabs).use(ElTabPane).use(ElButton).use(ElInput).use(ElForm).use(ElFormItem).use(ElRow)
    ...
    .use(ElOption).use(ElColorPicker)
}
