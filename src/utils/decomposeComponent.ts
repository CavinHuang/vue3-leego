...
export default function decomposeComponent(component: JsonUnknown, editorRect: JsonUnknown, parentStyle: JsonUnknown) {
  const selector = `#component${component.id}` as string
  const componentRect: any = $(selector)?.getBoundingClientRect()
  // 获取元素的中心点坐标
  const center = {
